...
inp = driver.find_element_by_xpath('//*[@id="hdtb-msb"]/div[1]/div/div[2]/a').click()

client = pymongo.MongoClient('mongodb+srv://sivaram:sivaram@cluster0.oigz9.mongodb.net/web?retryWrites=true&w=majority')

for i in range(20):
    img = driver.find_element_by_xpath('//*[@id="islrg"]/div[1]/div[{}]/a[1]/div[1]/img'.format(i+1))
    #print(img.get_attribute('src'))z
    imgmain=str(img.get_attribute('src'))
    ...
    img = db.images
    post={"img":str(imgmain)}
    id = img.insert_one(post).inserted_id
    print(id)
